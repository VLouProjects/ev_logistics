        ...
        >
          {["姓名", "公司", "電郵", "電話", "車輛需求"].map((label) => (
            <label key={label} className="block">
              ...
              <input className="w-full rounded-lg border border-border bg-bg px-3 py-3 text-gray-900 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" />
            </label>
          ))}

          <label className="block">
            <div className="mb-2 text-sm font-medium text-gray-600">是否需要冷鏈配置？</div>
            <select className="w-full rounded-lg border border-border bg-bg px-3 py-3 text-gray-900 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary">
              <option>尚未確定</option>
              <option>需要</option>
              <option>不需要</option>
            </select>
